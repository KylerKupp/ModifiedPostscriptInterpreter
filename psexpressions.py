...
class PSName(Expr):
    ...
    def eval(self,ps_env):
        ...
        if self.var_name[0] == '/':
            ...
            value = ps_env.lookup(self.var_name)
            if isinstance(value, CodeArrayValue):
                static_link = ps_env.get_static_link(self.var_name, len(ps_env.dictstack) - 1)
                value.apply(ps_env, static_link)
            else:
                ps_env.opPush(value)
        return      
...
class CodeArrayValue(Value):
    ...
    def apply(self, ps_env, static_link):
        """`ps_env` is the PSOperators object that include the `opstack` and `dictstack`. """
        ps_env.dictstack.append(tuple([static_link, {}]))
